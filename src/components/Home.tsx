    ...
    <div>
      ...
      <section className="mx-auto max-w-6xl px-5 pt-14 pb-16 grid md:grid-cols-[1.1fr_0.9fr] gap-12 items-start">
        <div>
          <p className="font-mono text-[11px] uppercase tracking-[0.16em] text-gold mb-4">
            {t("রাজশাহী বিভাগ · খণ্ড ০১")}
          </p>
          <h1 className="font-display text-[44px] sm:text-[58px] leading-[1.05] text-ink">
            {t("রাজশাহীর")} <em className="not-italic text-pine">{t("প্রতিটি চেম্বার")}</em>,
            <br />
            {t("এক জায়গায় লেখা আছে।")}
          </h1>
          <p className="mt-6 text-lg text-ink/70 max-w-md leading-relaxed">
            {t("বিশেষজ্ঞ, চেম্বারের ঠিকানা, ভিজিটিং আওয়ার আর ফি — সব একসাথে, যেন পুরনো দিনের ডাক্তার-ডায়েরির মতোই নির্ভরযোগ্য।")}
          </p>
          ...
          <div className="mt-9 bg-white border border-line shadow-[4px_4px_0_0_#123832] max-w-lg">
            <div className="flex items-center gap-3 px-4 py-3.5 border-b border-line">
              ...
              <input
                ...
                onClick={() => go("doctors")}
                value=""
                placeholder={t("ডাক্তারের নাম বা স্পেশালিটি লিখুন...")}
            ...
            >
              <span className="flex items-center gap-2">
                <MapPin className="h-4 w-4" strokeWidth={2} /> {t("রাজশাহীর মধ্যে খুঁজুন")}
              </span>
          ...
          <div className="mt-8 flex gap-8 font-mono">
            <div>
              ...
              <p className="text-[11px] uppercase tracking-wide text-ink/50">{t("ডাক্তার")}</p>
            </div>
            <div>
              <p className="text-2xl text-pine num">{cityStats.hospitals}</p>
              <p className="text-[11px] uppercase tracking-wide text-ink/50">{t("হাসপাতাল")}</p>
            </div>
            <div>
              <p className="text-2xl text-pine num">{cityStats.specialties}</p>
              <p className="text-[11px] uppercase tracking-wide text-ink/50">{t("বিভাগ")}</p>
            </div>
          </div>
        </div>
            ...
            {specialties.slice(0, 8).map((s, i) => (
              <li key={s.id}>
                ...
                >
                  ...
                  <span className="text-xs font-mono text-ink/40 num">{s.count} {t("জন")}</span>
                </button>
              </li>
            ))}
          </ul>
      ...
      <section className="mx-auto max-w-6xl px-5 py-14 border-t border-line">
        <div className="flex items-end justify-between mb-8">
          <div>
            <p className="font-mono text-[11px] uppercase tracking-[0.16em] text-gold mb-2">{t("এই সপ্তাহে বেশি খোঁজা হয়েছে")}</p>
            <h2 className="font-display text-3xl text-ink">{t("পরিচিত বিশেষজ্ঞগণ")}</h2>
          </div>
          <button onClick={() => go("doctors")} className="hidden sm:flex items-center gap-1.5 text-sm font-medium text-pine hover:gap-2.5 transition-all">
            {t("সব ডাক্তার দেখুন")} <ArrowRight className="h-4 w-4" strokeWidth={2} />
          </button>
        </div>
      ...
      <section className="mx-auto max-w-6xl px-5 py-14 border-t border-line">
        <div className="flex items-end justify-between mb-8">
          <div>
            <p className="font-mono text-[11px] uppercase tracking-[0.16em] text-gold mb-2">{t("চেম্বার ও চিকিৎসা কেন্দ্র")}</p>
            <h2 className="font-display text-3xl text-ink">{t("রাজশাহীর হাসপাতাল")}</h2>
          </div>
          <button onClick={() => go("hospitals")} className="hidden sm:flex items-center gap-1.5 text-sm font-medium text-pine hover:gap-2.5 transition-all">
            {t("সব হাসপাতাল দেখুন")} <ArrowRight className="h-4 w-4" strokeWidth={2} />
          </button>
        </div>
          ...
          {hospitals.map((h) => (
            ...
            >
              ...
              <span className="hidden sm:flex items-center gap-1 text-xs font-mono text-ink/40 num shrink-0">
                {h.doctorCount} {t("জন ডাক্তার")} <ArrowRight className="h-3.5 w-3.5" strokeWidth={2} />
              </span>
            </button>
          ))}
        </div>
      ...
      <section className="mx-auto max-w-6xl px-5 py-14 border-t border-line">
        <div className="bg-pine text-paper px-8 py-10 sm:px-12 sm:py-12 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div>
            <p className="font-mono text-[11px] uppercase tracking-[0.16em] text-gold mb-2">{t("আপনি কি একজন চিকিৎসক?")}</p>
            <h3 className="font-display text-2xl sm:text-3xl">{t("আপনার চেম্বার তালিকাভুক্ত করুন, বিনামূল্যে।")}</h3>
          </div>
